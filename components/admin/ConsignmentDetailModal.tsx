...
import { Consignment, ConsignmentUpdate, ConsignmentConfig } from '../../types';
...
const ConsignmentDetailModal: React.FC<ConsignmentDetailModalProps> = ({ isOpen, onApproveAndCreateListing, onDataUpdate, siteConfig, ...props }) => {
    ...
    const handleNumericFieldChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const isRate = name === 'custom_commission_rate';
        const parsedValue = isRate ? parseFloat(value.replace(/,/g, '.')) : parseInt(value.replace(/\D/g, ''), 10);
        setConsignment(prev => ({...prev, [name]: value === '' ? null : isNaN(parsedValue) ? prev[name as keyof Consignment] : parsedValue }));
    };

    const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const { name, checked } = e.target;
        setConsignment(prev => ({ ...prev, [name]: checked }));
    };
    ...
    const handleSaveFinancials = () => {
         handleUpdate({
            ...
            commission_enabled: consignment.commission_enabled,
            custom_commission_rate: consignment.custom_commission_rate
         });
    };
    
    const commissionRate = ((consignment.custom_commission_rate ?? siteConfig.commissionRate) || 0) / 100;
    const finalPrice = consignment.final_agreed_price || 0;
    const commissionAmount = (consignment.commission_enabled ?? true) ? finalPrice * commissionRate : 0;
    const netToOwner = finalPrice - commissionAmount;
                            ...
                            {activeTab === 'details' && (
                                <div className="space-y-6 animate-fade-in">
                                    ...
                                    <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 space-y-4">
                                        <h4 className="font-bold text-slate-800 dark:text-slate-200">Datos Financieros</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div><label htmlFor="appraisal_value" className="text-xs font-medium">Valor de Tasación</label><input type="text" id="appraisal_value" name="appraisal_value" value={consignment.appraisal_value?.toLocaleString('es-AR') || ''} onChange={handleNumericFieldChange} className="form-input mt-1 w-full" placeholder="ARS"/></div>
                                            <div><label htmlFor="market_price_suggestion" className="text-xs font-medium">Precio Mercado Sugerido</label><input type="text" id="market_price_suggestion" name="market_price_suggestion" value={consignment.market_price_suggestion?.toLocaleString('es-AR') || ''} onChange={handleNumericFieldChange} className="form-input mt-1 w-full" placeholder="ARS"/></div>
                                            <div className="md:col-span-3"><label htmlFor="final_agreed_price" className="text-xs font-medium">Precio Final Acordado</label><input type="text" id="final_agreed_price" name="final_agreed_price" value={consignment.final_agreed_price?.toLocaleString('es-AR') || ''} onChange={handleNumericFieldChange} className="form-input mt-1 w-full" placeholder="ARS"/></div>
                                        </div>
                                        <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                            <div className="flex items-center justify-between mb-3">
                                                <h5 className="font-semibold text-slate-700 dark:text-slate-300">Gestión de Comisión</h5>
                                                <label className="flex items-center gap-2 cursor-pointer text-sm">
                                                    <input type="checkbox" id="commission_enabled" name="commission_enabled" checked={consignment.commission_enabled ?? true} onChange={handleCheckboxChange} className="h-4 w-4 rounded border-gray-300 text-rago-burgundy focus:ring-rago-burgundy-darker" />
                                                    <span>Aplicar Comisión</span>
                                                </label>
                                            </div>
                                            {(consignment.commission_enabled ?? true) && (
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        <label htmlFor="custom_commission_rate" className="text-xs font-medium">Tasa Comisión Personalizada (%)</label>
                                                        <input type="number" step="0.1" id="custom_commission_rate" name="custom_commission_rate" value={consignment.custom_commission_rate ?? ''} onChange={handleNumericFieldChange} className="form-input mt-1 w-full" placeholder={`Global: ${siteConfig.commissionRate}%`} />
                                                    </div>
                                                </div>
                                            )}
                                            {finalPrice > 0 && (consignment.commission_enabled ?? true) && (
                                                <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                                                    <div className="bg-slate-100 dark:bg-slate-800 p-2 rounded">
                                                        <p className="font-medium text-slate-500">Comisión a Cobrar</p>
                                                        <p className="font-bold text-lg text-green-600">{commissionAmount.toLocaleString('es-AR', { style: 'currency', currency: 'ARS' })}</p>
                                                    </div>
                                                    <div className="bg-slate-100 dark:bg-slate-800 p-2 rounded">
                                                        <p className="font-medium text-slate-500">Neto a Pagar al Propietario</p>
                                                        <p className="font-bold text-lg text-slate-800 dark:text-white">{netToOwner.toLocaleString('es-AR', { style: 'currency', currency: 'ARS' })}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                        <button onClick={handleSaveFinancials} disabled={isSaving} className="px-3 py-1.5 text-sm font-semibold text-white bg-rago-burgundy rounded-md hover:bg-rago-burgundy-darker disabled:opacity-50">Guardar Datos Financieros</button>
                                    </div>
                                    <div>
                                        <p className="font-medium text-slate-500 dark:text-slate-400 mb-2">Imágenes</p>
                                        <div className="flex flex-wrap gap-2">{consignment.images.map((img, index) => <button onClick={() => setViewerState({ isOpen: true, startIndex: index })} key={img} className="rounded-md overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rago-burgundy"><img src={optimizeUrl(img, {w: 120, h: 90, fit: 'cover'})} className="w-28 h-20 object-cover" /></button>)}</div>
                                    </div>
                                </div>
                            )}
