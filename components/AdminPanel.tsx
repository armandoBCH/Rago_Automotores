...
const AnalyticsDashboard: React.FC<{ vehicles: Vehicle[], events: AnalyticsEvent[], isLoading: boolean, dbError: string | null, onRefresh: () => void, onReset: () => void }> = ({ vehicles, events, isLoading, dbError, onRefresh, onReset }) => {
    ...
    const analyticsSummary = useMemo(() => {
        ...
        events.forEach(event => {
            ...
            } else if (event.event_type === 'click_instagram') {
                instagramClicks++;
            }
        });
    ...
    if (dbError) return (
        <div className="bg-red-100 dark:bg-red-900/20 border-l-4 border-red-500 text-red-700 dark:text-red-300 p-4 rounded-r-lg" role="alert">
            <p className="font-bold">Error al Cargar Estadísticas</p><p>{dbError}</p>
             <button onClick={onRefresh} className="mt-3 px-3 py-1 text-sm font-semibold text-white bg-red-600 rounded-md hover:bg-red-700">Reintentar</button>
        </div>
    );
    ...
    return (
        <div className="space-y-8">
            ...
            <ConfirmationModal isOpen={showResetConfirm} onClose={() => setShowResetConfirm(false)} onConfirm={handleResetConfirm} title="¿Estás seguro?" message="Estás a punto de borrar TODAS las estadísticas. Esta acción no se puede deshacer."/>
            {showPasswordModal && <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-[60] p-4" onClick={() => setShowPasswordModal(false)}><div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleFinalReset} className="mt-4 space-y-4">
                    ...
                    <div className="flex justify-end space-x-3"><button type="button" onClick={() => setShowPasswordModal(false)} className="px-4 py-2 text-base font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-300 dark:hover:bg-gray-500">Cancelar</button><button type="submit" disabled={isResetting} className="px-4 py-2 text-base font-medium text-white bg-red-600 rounded-md hover:bg-red-700 disabled:bg-red-400 disabled:cursor-not-allowed flex items-center gap-2">
                        {isResetting && <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>}
...
const AdminPanel: React.FC<AdminPanelProps> = ({ vehicles, allEvents, onAdd, onEdit, onDelete, onLogout, onAnalyticsReset, onShowStats }) => {
    ...
    const filteredVehicles = useMemo(() => {
        ...
        if (!lowercasedTerm) return vehicles;
        return vehicles.filter(v => `${v.make} ${v.model} ${v.year}`.toLowerCase().includes(lowercasedTerm));
    }, [vehicles, searchTerm]);
    
    const TabButton: React.FC<{ tabId: 'inventory' | 'analytics', children: React.ReactNode }> = ({ tabId, children }) => (
        <button onClick={() => setActiveTab(tabId)} className={`px-4 py-2 font-semibold text-lg rounded-t-lg transition-colors duration-200 ${ activeTab === tabId ? 'bg-white dark:bg-slate-900 text-rago-burgundy' : 'bg-transparent text-slate-500 dark:text-slate-400 hover:text-rago-burgundy' }`} >
            {children}
        </button>
    );
        ...
        <div className="transition-colors duration-300">
             <div className="flex justify-between items-end mb-4 sm:mb-0">
                <div className="border-b border-slate-300 dark:border-slate-700">
                 ...
                 <button onClick={onLogout} className="flex items-center justify-center gap-2 px-4 py-2.5 text-base font-medium text-rago-burgundy dark:text-gray-300 bg-rago-burgundy/10 rounded-lg hover:bg-rago-burgundy/20 dark:bg-gray-800 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rago-burgundy transition-colors flex-shrink-0" aria-label="Cerrar sesión">
                    ...
                    <>
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                            ...
                            <div className="w-full sm:w-auto flex items-stretch gap-3">
                                <div className="flex-grow sm:w-64"><div className="bg-slate-200 dark:bg-gray-800/60 p-1 rounded-xl shadow-inner relative"><span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><SearchIcon className="h-5 w-5 text-gray-400" /></span><input type="text" placeholder="Buscar por marca, modelo..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 text-base bg-white dark:bg-gray-900 border-transparent text-gray-800 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:border-transparent focus:ring-2 focus:ring-rago-burgundy rounded-lg transition" aria-label="Buscar vehículo"/></div></div>
                                <button onClick={onAdd} className="flex items-center justify-center gap-2 px-5 py-2.5 text-base font-medium text-white bg-rago-burgundy rounded-lg hover:bg-rago-burgundy-darker focus:outline-none focus:ring-4 focus:ring-rago-burgundy/50 transition-colors flex-shrink-0"><PlusIcon className="h-5 w-5" />Añadir</button>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    {filteredVehicles.map(vehicle => (
                                        <tr key={vehicle.id} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                                            <td className="px-6 py-4"><img src={optimizeUrl(vehicle.images?.[0], { w: 80, h: 64, fit: 'cover', output: 'webp', q: 70 })} alt={`${vehicle.make} ${vehicle.model}`} className="w-20 h-16 object-cover rounded bg-gray-200 dark:bg-gray-700" loading="lazy" decoding="async"/></td>
                                            <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-base">{vehicle.make} {vehicle.model}</th>
                        ...
                        {filteredVehicles.length === 0 && (
                            <div className="text-center py-12 text-gray-500 dark:text-gray-400">{vehicles.length === 0 ? "No hay vehículos en el inventario." : `No se encontraron vehículos para "${searchTerm}".`}</div>
                        )}
                    </>
                )}
                {activeTab === 'analytics' && <AnalyticsDashboard vehicles={vehicles} events={allEvents} isLoading={analyticsLoading} dbError={analyticsError} onRefresh={onAnalyticsReset} onReset={onAnalyticsReset} />}
            </div>
        </div>
    );
};
