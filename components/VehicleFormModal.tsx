    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialData) {
                ...
                // Draft logic for new vehicles can be added here if needed
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const uploadPromises = newFilesToUpload.map(async (imageFile) => {
            ...
            try {
                ...
                const { error: uploadError } = await supabase.storage.from('vehicle_images').uploadToSignedUrl(path, signedUrl, compressedFile);
                if (uploadError) throw uploadError;
                
                setSubmitProgress(prev => ({ ...prev, completed: prev.completed + 1 }));

                const { data: { publicUrl } } = supabase.storage.from('vehicle_images').getPublicUrl(path);
                return publicUrl;
            } catch (error) {
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8">
                        <form id="vehicle-form" onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="col-span-full flex items-center gap-3 pt-2">
                                <input id="is_featured" name="is_featured" type="checkbox" checked={formData.is_featured} onChange={e => setFormData(prev => ({ ...prev, is_featured: e.target.checked }))} className="h-5 w-5 rounded border-gray-300 text-rago-burgundy focus:ring-rago-burgundy-darker dark:bg-gray-700 dark:border-gray-600" />
                                <label htmlFor="is_featured" className="block text-base font-medium text-gray-700 dark:text-gray-300">Marcar como vehículo destacado</label>
                            </div>
                            <div>
...
const VehicleDetailPreview: React.FC<{ vehicle: Vehicle }> = ({ vehicle }) => (
    <div className="border rounded-lg dark:border-gray-600 bg-gray-50 dark:bg-gray-800 shadow-inner">
        <div className="max-h-[calc(90vh - 160px)] overflow-y-auto preview-scrollbar">
            <div className="aspect-[4/3] bg-gray-200 dark:bg-black"><ImageCarousel images={vehicle.images} /></div>
